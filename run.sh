...
if [[ $USE_DOCKER == 0 ]]; then
    ...
    if [[ $KILL_PS == 1 ]]; then
        kill -9 $(ps aux | grep './avatarify/afy/cam_fomm.py' | awk '{print $2}') 2> /dev/null
    fi
    
    source ./avatarify/scripts/settings.sh
    ...
    python ./avatarify/afy/cam_fomm.py \
        --config $FOMM_CONFIG \
    ...
    if [[ $IS_WORKER == 0 ]]; then
        ...
        docker run $DOCKER_ARGS -it --rm --privileged  \
            ...
            avatarify python3 ./avatarify/afy/cam_fomm.py \
                --config $FOMM_CONFIG \
                --checkpoint $FOMM_CKPT \
                --virt-cam $CAMID_VIRT \
                --relative \
